    ...
    def complete_classes(self):
        ...
        def process_class(cls):
            if cls.name not in processed:
                ...
                super_ids = []
                if cls.superclass:
                    supercls = by_name[cls.superclass]
                    sid = supercls.id
                    if sid not in self.class_children:
                        self.class_children[sid] = set()
                    self.class_children[sid].add(cls.id)
                    process_class(supercls)
                    super_attrs = supercls.attributes
                    super_ids = supercls.attrs

                if cls.attributes:
                    cls.attributes = super_attrs + cls.attributes
                    cls.attrs = super_ids + [a.id for a in cls.attributes]
                    for attr in cls.attributes:
                        self.add(attr)

                else:
                    cls.attrs = super_ids + cls.attrs
                    cls.attributes = [attr_by_id[a] for a in cls.attrs]
